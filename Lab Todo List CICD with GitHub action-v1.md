...
```plaintext
- กรณีที่ 1: ฟังก์ชันคำนวณทั่วไป
ทดสอบ Logic ตรงๆ ว่าใส่ Input แล้วได้ Output ที่ถูกต้องหรือไม่
Code การทำงาน: ฟังก์ชันรับราคา แล้วคำนวณ VAT 7%
        def calculate_price_with_vat(price):
            return price * 1.07

Code ทดสอบ: ตรวจสอบว่าถ้าใส่ 100 ต้องได้ผลลัพธ์เป็น 107
        def test_calculate_price_with_vat():
            assert calculate_price_with_vat(100) == 107.0

- กรณีที่ 2: ฟังก์ชัน API Endpoint
ทดสอบว่าเมื่อมีการยิง Request เข้ามาที่ API จะได้ผลลัพธ์ที่ถูกต้องหรือไม่
Code การทำงาน: API endpoint ที่คืนข้อมูล user
        @app.route("/api/users/<user_id>")
        def get_user_profile(user_id):
            user = DUMMY_USERS.get(user_id)
            return jsonify(user), 200

Code ทดสอบ: จำลองการยิง Request แล้วเช็ค Status Code และข้อมูลที่ได้
...
- กรณีที่ 3: ฟังก์ชันที่เรียกใช้ส่วนอื่น (Mocking)
ทดสอบฟังก์ชันที่ต้องพึ่งพา Database หรือ Service อื่น โดยจำลอง (Mock) ส่วนนั้นขึ้นมา
Code การทำงาน: ฟังก์ชันที่ต้องดึงข้อมูลจาก Database
        def get_welcome_message(user_id):
            user = db_connector.get_user_by_id(user_id) # เรียกใช้ DB
            return f"Welcome, {user['name']}!"

Code ทดสอบ: ใช้ Mock สร้าง db_connector ปลอมๆ เพื่อไม่ต้องต่อ DB จริง
        def test_get_welcome_message_user_exists(monkeypatch):
            # จำลองว่า DB คืนค่า {"name": "Charlie"} เสมอ
            def mock_get_user(id):
                return {"name": "Charlie"}
            monkeypatch.setattr("app.db_connector.get_user_by_id", mock_get_user)

            # ทดสอบฟังก์ชันหลัก
            assert get_welcome_message("any_id") == "Welcome, Charlie!"
```
...
- [ ✔️ ] Render Database สร้างเสร็จและสถานะ "Available"
- [ ✔️ ] Render Web Service deploy สำเร็จและสถานะ "Live"
- [ ✔️ ] Railway Database และ Web Service ทำงานปกติ
- [ ❌ ] สร้าง GitHub Secrets ครบ 5 ตัว
- [ ✔️ ] ทดสอบ health endpoints ของทั้ง Render และ Railway ได้
- [ ✔️ ] URL ไม่มี `/` ท้าย
- [ ✔️ ] DATABASE_URL ใช้ Internal URL (สำหรับ Render)
...
1. Docker Architecture:
   - เหตุใดจึงต้องแยก database และ application เป็นคนละ containers ?
        > เพื่อให้แต่ละส่วน อัปเดตและขยายระบบ (Scale) ได้อิสระ ไม่กระทบกัน

   - Multi-stage build มีประโยชน์อย่างไร?
        > เพื่อให้ Image สุดท้าย มีขนาดเล็กและปลอดภัยขึ้น โดยทิ้งไฟล์ที่ไม่จำเป็นออกไป

2. Testing Strategy:
   - การวัด code coverage มีความสำคัญอย่างไร?
        > เพื่อวัดว่าเทสของเราครอบคลุมโค้ดแค่ไหน และหาจุดที่ยังไม่ได้เทส

3. Deployment:
   - Health check endpoint มีความสำคัญอย่างไร?
        > เป็นจุดให้ระบบ เช็คว่าแอปยังทำงานอยู่หรือไม่ ถ้าล่มจะได้รีสตาร์ทให้อัตโนมัติ

   - Render และ Railway มีความแตกต่างกันอย่่างไร?
        > Render: ยืดหยุ่นสูง เหมาะกับงานซับซ้อน มี Free tier
        > Railway: ใช้ง่ายและเร็ว จ่ายตามจริงและมีเครดิตฟรีให้
```
